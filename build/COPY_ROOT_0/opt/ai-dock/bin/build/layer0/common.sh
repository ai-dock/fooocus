...
build_common_create_venv() {
    ...
    "python${PYTHON_VERSION}" -m venv "$FOOOCUS_VENV"
    "$FOOOCUS_VENV_PIP" install --no-cache-dir \
        ipykernel \
        ipywidgets
    "$FOOOCUS_VENV_PYTHON" -m ipykernel install \
        --name="fooocus" \
        --display-name="Python${PYTHON_VERSION} (fooocus)"
    # Add the default Jupyter kernel as an alias of fooocus
    "$FOOOCUS_VENV_PYTHON" -m ipykernel install \
        --name="python3" \
